...
FoodMat = nan(imsize(1), imsize(2),numel(tifFiles));
AntsMat = nan(imsize(1), imsize(2),numel(tifFiles));
...
for k=1:numel(tifFiles);
    try
        ...
        fprintf(1, 'Now reading %s\n', fullFileName);
        im = mosaicToRGB(single(imread(fullFileName))/2^16);
        FoodMat(:,:,k)=bsxfun(@times,(im(:,:,1)-median(I(axialROImask))).*(im(:,:,1)>threshFood), BW);
        AntsMat(:,:,k)=bsxfun(@minus, bsxfun(@times, squeeze(im(:,:,1)), BW)<threshAmIAnt,~BW);
